...
const CalibrateToPlate: FC<{ onDone: () => void, wellName: WellName }> = ({onDone, wellName}) => {
  const classes = useCalibrateToPlateStyles()
  const [moveDistance, setMoveDistance] = useState<Distance>(Distance.MM0_1)
  return <Fragment>
    <Typography className={classes.title} variant="h4"> Calibrate to well {wellName}</Typography>
    <div className={classes.controlGroup}>
      <FormControl component="fieldset">
        <FormLabel component="legend">Movement Distance</FormLabel>
        <RadioGroup aria-label="move-distance" name="move-distance" value={moveDistance} onChange={(e) => {
          setMoveDistance(Number(e.target.value) as Distance)
        }}>
          <FormControlLabel value={Distance.MM0_1} control={<Radio/>} label="0.1 mm"/>
          <FormControlLabel value={Distance.MM1} control={<Radio/>} label="1 mm"/>
          <FormControlLabel value={Distance.MM10} control={<Radio/>} label="10 mm"/>
        </RadioGroup>
      </FormControl>
      <div className={classes.keyContainer}>
        <Button className={classes.buttonUp} variant="outlined"><ArrowUpward/></Button>
        <Button className={classes.buttonDown} variant="outlined"><ArrowDownward/></Button>
        <Button className={classes.buttonLeft} variant="outlined"><ArrowBack/></Button>
        <Button className={classes.buttonRight} variant="outlined"><ArrowForward/></Button>
      </div>
    </div>
    <div className={classes.centerButtonDiv}>
      <Button onClick={() => {
        onDone()
      }} color="primary" size="large" variant="outlined">Next</Button>
    </div>
  </Fragment>
}

const filter = createFilterOptions<string>();
const ConnectToDeviceStep: FC<{ onDone: () => void, openLoader: () => void, closeLoader: () => void }> = ({onDone, openLoader, closeLoader}) => {
  ...
  const [ipAddressOptions, setIpAddressOptions] = useState<string[]>(getSavedOptions())
  ...
  const isValidIp = (ipAddr: string): boolean => (/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/).test(ipAddr)
  ...
  return <Fragment>
    ...
    <Button onClick={async () => {
      ...
      closeLoader()
      onDone()
    }} variant="outlined" color="primary">Connect</Button>
  </Fragment>
}
    ...
    <div className={classes.root}>
      ...
      <div>
        {activeStep === 5 ? (
          ...
          <div>
            {[
              <ConnectToDeviceStep closeLoader={closeLoader} onDone={handleNext} openLoader={openLoader}/>,
              <CalibrateToPlate onDone={handleNext} wellName={WellName.A1}/>,
              <CalibrateToPlate onDone={handleNext} wellName={WellName.A12}/>,
              <CalibrateToPlate onDone={handleNext} wellName={WellName.H12}/>,
